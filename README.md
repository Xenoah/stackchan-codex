...
3. Yaw `-128.0°`、`+128.0°`、Pitch `0.0°`、`90.0°`を低速で順番に検証
4. 各端点のフィードバック値を `stack_cal` 名前空間へ保存
5. サーボ電源をOFFにし、本体全体を水平な場所へ静置
6. BMI270ジャイロを8秒間校正し、300サンプルの加速度平均から水平の
   Roll/Pitchゼロ点を保存

各サーボ移動前に確認画面を表示します。移動中に画面へ触れると緊急停止します。
目標から8度以上ずれた場合、15秒以内に到達しない場合、IMUが動いた場合は失敗として
電源・トルクをOFFにします。
